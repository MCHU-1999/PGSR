...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        self.knn_dists = None
        self.knn_idx = None
        self._normal_signs = torch.empty(0)
    ...
    def restore(self, model_args, training_args):
        ...
        self._normal_signs = normal_signs
    ...
    def get_normal(self, view_cam=None):
        ...
        if self._normal_signs.numel() != 0:
            sign_cont = torch.tanh(self._normal_signs)
            sign_hard = torch.sign(sign_cont)
            sign_hard[sign_hard == 0] = 1.0
            sign = sign_cont + (sign_hard - sign_cont).detach()
            normal_global = normal_global * sign
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        init_signs = torch.ones((fused_point_cloud.shape[0], 1), device="cuda") * 3.0
        ...
        self._normal_signs = nn.Parameter(init_signs.requires_grad_(True))
    ...
    def load_ply(self, path):
        ...
        # initialize learnable normal sign logits to +1 for all points
        init_sign = torch.ones((self.get_xyz.shape[0], 1), device="cuda") * 3.0
        self._normal_signs = nn.Parameter(init_sign)
    ...
    def prune_points(self, mask):
        ...
        self._normal_signs = optimizable_tensors["normal_signs"]
    ...
    def densification_postfix(self, new_xyz, new_knn_f, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation):
        new_signs = torch.ones((new_xyz.shape[0], 1), device=new_xyz.device)
        d = {"xyz": new_xyz,
        "knn_f": new_knn_f,
