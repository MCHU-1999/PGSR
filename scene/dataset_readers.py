...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, fg_masks_folder):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        ...
        fg_mask_path = os.path.join(fg_masks_folder, os.path.basename(extr.name))
...
def readColmapSceneInfo(path, images, fg_masks, eval, llffhold=8):
    try:
    ...
    fg_masks_dir = "fg_masks" if fg_masks == None else fg_masks
    cam_infos_unsorted = readColmapCameras(
        cam_extrinsics=cam_extrinsics, 
        cam_intrinsics=cam_intrinsics, 
        images_folder=os.path.join(path, reading_dir),
        fg_masks_folder=os.path.join(path, fg_masks_dir)
    )
